...
- `VERSION_NAME` / `VERSION_CODE` in `gradle.properties` **track upstream** (currently `12.5.9` / `97`).
...
- Fork `versionName` = `"<VERSION_NAME>+<BUILD_NUMBER>"`, with the counter **zero-padded to three
  digits** (e.g. `12.5.9+001`) so builds sort in order in file lists — `+010` must land after
  `+009`, not before `+9`.
...
- Output APK filename = `shiroikuma-kuchusen_<VERSION_NAME>+<BUILD_NUMBER>_arm64-v8a.apk`
  (e.g. `shiroikuma-kuchusen_12.5.9+001_arm64-v8a.apk`).
